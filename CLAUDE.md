...
- `PM_TEMPLATE` is **set in Vercel Production** (and in `.env.local`), so `/aipm` **Connect GitHub** repo-generation works in prod. Added via `vercel env add PM_TEMPLATE production` — note the CLI needs BOTH `NODE_EXTRA_CA_CERTS` AND the nvm node bin on PATH (`export PATH=/Users/<you>/.nvm/versions/node/v24.15.0/bin:$PATH`), else it dies with `env: node: No such file or directory`.
